...
The named palettes are built from design tokens: `text`, `muted`, `accent`, `chart`, `good`, `warn`, and `error`. Green is the default; blue, brown, and yellow change the primary text, headings, and chart color. Allowance semantics remain stable: healthy is green, warning is orange, and critical is red. Brown and orange use the closest available terminal color when 256-color output is unavailable. Custom token values accept the basic terminal color names, `gray`, `brown`, `orange`, or a six-digit hexadecimal value.
