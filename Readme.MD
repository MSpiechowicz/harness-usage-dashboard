...
Section dividers separate token rate, session summaries, history, and account allowances. Compact mode removes blank separator rows, avoids repeating the total for a single provider, hides history when the project has no recorded usage, and omits a nonexistent previous session. Details mode retains per-model, per-thinking-level breakdowns and session IDs.
