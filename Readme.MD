...
**Your recorded usage history stays on your machine.** Session token records and observed quota changes are saved in a local SQLite database, not a hosted dashboard service. Settings are saved separately in local JSON files. See [Settings and privacy](#settings-and-privacy) for file locations and exactly what is stored.

[Install](#installation) · [Use](#usage) · [Themes](#theme-preview) · [Update](#updating) · [Uninstall](#uninstalling) · [Troubleshoot](#troubleshooting)

[![Green Compact and blue Details mockups showing token charts, session and project totals, thinking-level and cache breakdowns, allowance colors, and the command box](docs/assets/dashboard.png)](docs/assets/dashboard.png)

*Green Compact (left) and blue Details (right), arranged side by side at expanded widths for comparison. Mockups generated from the current dashboard row-rendering functions with **synthetic usage data** and an illustrative terminal palette—not live account readings or terminal captures. Details continues below the visible area by scrolling. No real conversation content, credentials, or account identifiers are included. Click to enlarge.*
...
- **Provider and model breakdowns:** Compact mode groups tokens by provider; Details separates each model by effective thinking level and includes input/output and cache-read/write counts. Unattributed task aggregates remain explicitly labelled.
...
- **Local storage:** recorded session usage and quota observations live in a SQLite database on your machine, scoped by project within each OMP profile. Provider selection, hidden rows, position, density, polling interval, visibility, and theme tokens persist in a separate local JSON file.
...
| **Section Visibility** | Choose Commands, Previous, or History, then Hide or Show | `/usage-dashboard history hide` |
...
**Section Visibility** controls each section independently in Compact and Details. All three are visible by default. Hiding Previous removes the previous-session section; hiding History removes both Other sessions and Project total. These settings only affect presentation: recording continues, stored usage is preserved, and Show restores the accumulated data.
...
| `/usage-dashboard history hide` | Hide History, including Other sessions and Project total. |
| `/usage-dashboard history show` | Restore History (visible by default when available). |
...
Section dividers separate token rate, session summaries, history, and account allowances. Every section has a blank separator row in Compact and Details modes; Details also separates model, history, and provider entries for readability. In Details, provider totals under Current/Previous session and the Other sessions/Project total subsections use theme-colored headings with totals at the right. Model names retain normal text contrast; input/output and cache-read/write rows use the secondary theme color, like Reset credits. Compact mode avoids repeating the total for a single provider, hides history when the project has no recorded usage, and omits a nonexistent previous session.

**Recorded history** is split into two views. **Other sessions** is the total of locally recorded requests from sessions other than the current one in the current Git repository (or current working directory when no repository root is found). **Project total** is the aggregate of all recorded requests in the current project, including the current session. In Details mode, both views expand to the model/thinking-level rows and token dimensions used by the session summaries. `/new` preserves both totals. Resuming the same conversation continues its record, while inherited fork context is excluded from new consumption. Large token totals use compact `k`, `M`, `B`,…
...
**The dashboard stores its recorded usage history locally on the machine running OMP.** It does not upload that history to a dashboard server or synchronize it between machines. Provider usage polling and GitHub release checks still require network requests; local storage does not mean the dashboard operates entirely offline.

### Local usage database

Session token records and observed account-quota changes are stored in **`usage-dashboard.sqlite3`**, a local SQLite database. There is one database per OMP profile, with project-scoped records inside it—not a separate database file for every project.
...
The database is created with owner-only permissions and updated transactionally as usage arrives, not just at shutdown. It stores hashed project scopes, session/request identifiers, timestamps, provider/model/thinking-level identities, numeric token counters, hashed task-group associations and original aggregate counters for deduplication, and quota observation summaries with hashed account/window identities. Charts and session/project totals are derived from those local records.

It does **not** store prompts, responses, tool output, credentials, raw account IDs, or raw project paths. A crash can lose usage not yet reported or saved, but does not erase already committed session history. Uninstalling the integration leaves this database intact.

### Local settings files

Preferences and release-check metadata also stay on your machine, but are stored in **JSON files, not inside the usage database**.

Provider order, hidden providers, window filters, position, display mode, Commands footer visibility, Previous session visibility, History visibility, polling interval, on/off state, and theme tokens are saved per profile:
